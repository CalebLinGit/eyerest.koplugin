...
local DEFAULTS = {
    ...
    mini_duration_minutes = 5,
    long_break_every = 3,
    long_duration_minutes = 15,
    postpone_minutes = 5,
}
...
function EyeRest:triggerBreak(force_type)
    ...
    local dur = (bt == "long" and self:get("long_duration_minutes")
        or self:get("mini_duration_minutes")) * 60
...
function EyeRest:statusLine()
    ...
    return T(_("Next break in about %1 min"), logic.coarseRemainingMin(self:remaining()))
end
...
function EyeRest:settingsItems()
    return {
        ...
        self:spinItem(_("Mini break duration: %1 min"), "mini_duration_minutes", 1, 60),
        self:spinItem(_("Long break: every %1 mini breaks (0=off)"), "long_break_every", 0, 10),
        self:spinItem(_("Long break duration: %1 min"), "long_duration_minutes", 1, 60),
        {
            text = _("Strict mode"),
